...
class KeychainPasswordTests: XCTestCase {
    ...
    func testAccessibility() {
        ...
        XCTAssertThrowsError(try genericPasswordKeychain.set(Constants.password, for: Constants.key, with: (accessibility, nil, nil)), "") { error in
        ...
        XCTAssertThrowsError(try internetPasswordKeychain.set(Constants.password, for: Constants.key, with: (accessibility, nil, nil)), "") { error in
            switch error as? KeychainItemError {
            case .alreadyExistWithOtherAccessibility: return
            default: XCTFail("incorrect error")
            }
